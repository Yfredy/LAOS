...
### 裁定 1：~316M 的 wav2vec2/HuBERT/WavLM-large 归**中型**，候选池标错了

上面候选池把 `wav2vec2 / HuBERT / WavLM 的 large 版本` 和 `emotion2vec-plus-large`
列在了「大型线」——**这是候选池的错，不是口径的错**。

按本文件「规模分档」表，中型是 **30M–500M（左闭右开）**，316M 明确落在中型。
裁定：**以分档表为准**，这些模型收录进 `02-medium-models.md`；候选池此处保留原文
不加修改，以此节作为更正记录，避免后人再按候选池的错误归类去核对。

推论：真正的「大型档」（>500M）几乎被**语音大模型 / LLM 路线**占满，
纯卷积/Transformer 编码器很少超过 500M。这一点在 landscape 交叉表里要体现出来。

### 裁定 2：emotion2vec-plus-large 的参数量存在来源冲突，但不影响分档

- 官方标称 **~300M**
- IJCNLP 2025（arXiv 2506.06820）独立标注 **164M**；按 emotion2vec base 已知
  93.79M 做字节/参数标定反推得 ~162M；其 config 为 depth 8 / dim 1024，撑不到 300M

裁定：表中记 **164**，并在正文写明冲突与双方出处。**两个值都 < 500M，分档不受影响**
