...
# --------------------------------------------------------------------------
# mic 四级能力阶梯（调研 §8.4：高层含低层、默认收紧、人类一次性释放）---------
# 顺序即包含关系：mic.listen ⊂ mic.record ⊂ mic.transcribe ⊂ mic.always_on；
# 授权以"级"为单位，持有所需级或更高级即放行，无任何相关级一律拒绝。
...
class AgentKernel:
    ...
    async def syscall(self, pid: int, tool: str, args: dict | None = None,
        ...
        # mic 能力阶梯：录音类工具在未持任何相关级别时，拒绝理由给出
        # 阶梯语义 EACCES（含所需级别名），而非笼统 EPERM
        if not self._effective_allows(pcb, tool, consume=True):
            req = self._mic_required(tool)
            if req is not None:
                return self._deny(
                    pcb, tool, args, started,
                    f"EACCES: mic capability level {req} not granted")
            return self._deny(pcb, tool, args, started, "EPERM: capability not granted")

        # mic 四级能力阶梯闸门：即便 raw 工具名在自身 caps / 委托里命中，
        # 未持有对应级别（或更高级）一律拒绝——授权以级别为单位，默认收紧
        ...
        if entry is None:
            ...
            # 隐私红线：成功派发到驱动的每一次 mic.* / rec.* syscall 都额外
            # 落一条 event:"mic" 审计记录（被内核 _deny 拒绝的调用由 _deny
            # 补记，denied:true）——录音行为无论成败都必须可追责、可计数
            if tool.startswith(MIC_AUDIT_PREFIXES):
    ...
    def _mic_ladder_ok(self, pcb: PCB, tool: str) -> bool:
        ...
        有效能力 = 自身 caps ∪ 未过期委托：委托的级别同样参与蕴含，
        避免"父授子 mic.record 级、子反被阶梯拦下"的语义断裂。"""
        req = self._mic_required(tool)
        if req is None:
            return True
        idx = MIC_LEVEL_ORDER.index(req)
        levels = MIC_LEVEL_ORDER[idx:]
        if any(pcb.caps.allows(c) for c in levels):
            return True
        return any(d["remaining"] > 0 and d["caps"].allows(c)
        ...
        for d in self._delegations.get(pcb.pid, []):
            if d["remaining"] > 0 and d["caps"].allows(tool):
                if consume:
                    d["remaining"] -= 1
                return True
        # mic 能力阶梯：持有所需级或更高级即蕴含该工具（高层含低层）——
        # spawn 授权以级别为单位时，无需把级内工具名逐个重复授一遍。
        # 仅对阶梯覆盖的录音类工具生效，其余工具不得借道绕过 raw 检查
        if self._mic_required(tool) is not None:
            return self._mic_ladder_ok(pcb, tool)
        return False
    ...
    def _deny(self, pcb: PCB, tool: str, args: dict, started: float, err: str):
        ...
        # 隐私红线补口子：mic.* / rec.* 被内核拒绝（EPERM/EACCES/EDQUOT 等）
        # 同样要留下 event:"mic" 审计——录音意图无论成败都可追责、可计数
        if tool.startswith(MIC_AUDIT_PREFIXES):
            self.audit.write({"t": time.time(), "event": "mic",
