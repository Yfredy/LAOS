...
class EarServerChannelTest(unittest.TestCase):
    ...
    def test_transcribe_unified_json_shape(self):
        # mic 能力阶梯（Task 1）：ear.transcribe 属 mic.transcribe 级，
        # 仅授 ear.* 不再放行，spawn caps 须补级别
        pcb = self._spawn(["ear.*", "mic.transcribe"])
...
class TestFunasrReal(unittest.TestCase):
    ...
    def test_transcribe_silence_ok_with_text_key(self):
        # mic 能力阶梯（Task 1）：ear.transcribe 须持 mic.transcribe 级
        pcb = self.kernel.spawn(name="a", caps=["ear.*", "mic.transcribe"],
                                ctx=ContextManager(system_prompt="t"))
...
class TestMicDeniedStillAudited(unittest.TestCase):
    ...
    def test_denied_mic_record_writes_both_records(self):
        ...
        # mic 能力阶梯（Task 1）：无任何 mic 级 → 阶梯语义 EACCES（原为 EPERM）
        self.assertIn("EACCES: mic capability level mic.record", res.error)
