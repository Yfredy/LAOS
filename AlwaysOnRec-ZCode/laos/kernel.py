...
def _in_time_window(now: str, window: str) -> bool:
    """now / window 均为 HH:MM；window 形如 "09:00-18:00"，支持跨零点（22:00-06:00）。

    非法窗口串按不匹配处理（fail-closed：写错 scope 不应意外放行）。
    """
    try:
        start_s, end_s = window.split("-", 1)
        cur, start, end = _hhmm_to_min(now), _hhmm_to_min(start_s), _hhmm_to_min(end_s)
    except (ValueError, AttributeError):
        return False
    if start <= end:
        return start <= cur <= end
    return cur >= start or cur <= end  # 跨零点：cur 在 [start,24:00) 或 [00:00,end]
...
class AgentKernel:
    ...
    async def syscall(self, pid: int, tool: str, args: dict | None = None,
        ...
        # time 作用域：mic.* 限制在 task_scope 的 time:HH:MM-HH:MM 时间窗内
        # （Apple Siri Recap 式"何时可听"调度——能力说"能用麦克风"，
        # 任务说"哪些钟点可以听"；支持跨零点区间如 22:00-06:00）
        time_scopes = [s[5:] for s in (pcb.task_scope or []) if s.startswith("time:")]
        if time_scopes and tool.startswith("mic."):
            if not any(_in_time_window(self._now_hhmm(), w) for w in time_scopes):
                return self._deny(pcb, tool, args, started,
