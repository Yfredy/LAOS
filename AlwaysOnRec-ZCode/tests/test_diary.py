...
class TestBuildDiary(unittest.TestCase):
    ...
    def test_heard_section_renders_title_line(self):
        """journal 条目带 Apple 式标题时，"今天听到的"渲染标题且不带 # 前缀。"""
        self.store.remember("journal",
                            "# 医生说指标好转\n[14:30] 今天下午三点和医生讨论了复查安排。",
                            tags=["NEUTRAL"])
        result = self._build(self.records)
        heard = result["五、今天听到的"]
        self.assertIn("医生说指标好转", heard)
        self.assertNotIn("# 医生说指标好转", heard)
